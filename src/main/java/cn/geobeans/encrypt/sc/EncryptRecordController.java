...
public class EncryptRecordController {
    ...
    private final EncryptRecordService service;

    public EncryptRecordController(@Lazy EncryptRecordService service) {
    ...
    public JsonResponse getList(@ApiParam("jar包名称") @RequestParam(value = "name", required = false) String name) {
        return new JsonResponse(service.findAllList(name));
    }
